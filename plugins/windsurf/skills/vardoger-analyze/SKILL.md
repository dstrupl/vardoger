...
description: "Use when the user asks to personalize their assistant, to use vardoger, or to analyze their Windsurf conversation history. Runs the vardoger CLI to read past conversations and generate tailored instructions."
---
...
```bash
vardoger status --platform windsurf --json
```
...
```bash
vardoger prepare --platform windsurf
```
...
```bash
vardoger prepare --platform windsurf --batch 1
```
...
```bash
vardoger prepare --platform windsurf --synthesize
```
...
```bash
echo "YOUR_PERSONALIZATION_HERE" | vardoger write --platform windsurf --scope global
```
