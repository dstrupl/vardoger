...
- [x] Plugin packaging and marketplace submission for Claude Code (community catalog + custom marketplace) — **community catalog live** via the [`anthropics/claude-plugins-community`](https://github.com/anthropics/claude-plugins-community) mirror (submitted 2026-04-20 through the `claude.ai/settings/plugins/submit` form, landed in the [`4749e7a`](https://github.com/anthropics/claude-plugins-community/commit/4749e7a) sync on 2026-04-28 pinned at SHA `da14439`; this is the catalog Claude Code clients read via the in-product `/plugin` Discover tab); **custom marketplace live (self-served)** via `.claude-plugin/marketplace.json` at the repo root (users install with `/plugin marketplace add dstrupl/vardoger` → `/plugin install vardoger@vardoger`, pointing at `plugins/claude-code/` via a `./plugins/claude-code` relative source — the same pattern Anthropic's own catalog plugins use). A separate, curated tier-2 directory at `anthropics/claude-plugins-official` exists but has no public submission flow and is tracked watch-only in [`MARKETPLACE_STATUS.md`](./MARKETPLACE_STATUS.md); it is not considered a Phase 4 deliverable.
