...
- [x] Plugin packaging and marketplace submission for Claude Code (community catalog + custom marketplace) — **custom marketplace live (self-served)** via `.claude-plugin/marketplace.json` at the repo root (users install with `/plugin marketplace add dstrupl/vardoger` → `/plugin install vardoger@vardoger`, pointing at `plugins/claude-code/` via a `./plugins/claude-code` relative source — the same pattern Anthropic's own catalog plugins use); **community catalog regressed 2026-05-15** — was Live in the [`anthropics/claude-plugins-community`](https://github.com/anthropics/claude-plugins-community) mirror from 2026-04-28 ([sync `4749e7a`](https://github.com/anthropics/claude-plugins-community/commit/4749e7a), pinned at our SHA `da14439`) through 2026-05-01 ([sync `7a773c6`](https://github.com/anthropics/claude-plugins-community/commit/7a773c6)) and was removed in the 2026-05-13 catch-up sync ([PR #28](https://github.com/anthropics/claude-plugins-community/pull/28), *"Bulk sync: 666 plugin entries (453 updated, 4 added, 209 cleaned up)"*) along with 208 other entries; no per-row rationale published. Re-submission via [`platform.claude.com/plugins/submit`](https://platform.claude.com/plugins/submit) is owner-only (the form is behind Anthropic auth); tracked under "Owner-only actions" in [`MARKETPLACE_STATUS.md`](./MARKETPLACE_STATUS.md). The custom marketplace path remains the recommended install for users who do not want to wait on Anthropic's pipeline. A separate, curated tier-2 directory at `anthropics/claude-plugins-official` exists but has no public submission flow and is tracked watch-only in [`MARKETPLACE_STATUS.md`](./MARKETPLACE_STATUS.md); it is not considered a Phase 4 deliverable. Box stays `[x]` because both the original submission and the self-served install path were delivered; the upstream-side cull is being re-driven through the same form.
