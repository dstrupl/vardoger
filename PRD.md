...
- [x] Skill publishing to ClawHub for OpenClaw — **live (self-served)** as [`vardoger-analyze@0.3.0`](https://clawhub.ai/skills/vardoger-analyze) (published 2026-04-22, publish id `k9796s5r5hk5ea46kxbpwk49dd85axz8`, verified via `bunx --bun clawhub inspect vardoger-analyze`). See [`MARKETPLACE_STATUS.md`](./MARKETPLACE_STATUS.md) for the republish command.
- [ ] Plugin packaging and marketplace submission for GitHub Copilot CLI — **custom marketplace live (self-served)** via `plugins/copilot/marketplace.json` (Copilot CLI has no central registry for custom marketplaces — users install directly via `copilot plugin marketplace add dstrupl/vardoger:plugins/copilot`); **`awesome-copilot` submitted, awaiting review** ([PR #1461](https://github.com/github/awesome-copilot/pull/1461), retargeted to `staged` on 2026-04-21)
...
- [ ] Official MCP Registry submission (`registry.modelcontextprotocol.io`) — **not started — prep complete** (draft `server.json` at `plugins/mcp-registry/server.json`, `mcp-name: io.github.dstrupl/vardoger` marker landed in `README.md`; publishing requires a PyPI re-release so the marker ships in the package description, then `mcp-publisher publish`). Feed is ingested by Docker Desktop, VS Code MCP picker, and Windsurf Enterprise internal registries.
...
- [ ] Docker MCP Registry submission ([`docker/mcp-registry`](https://github.com/docker/mcp-registry)) — **prep complete — awaits 0.3.1 release.** Runtime `Dockerfile` landed at the repo root (multi-stage Python 3.12 slim, non-root UID 1000, builds vardoger from the pinned source commit, stdio `ENTRYPOINT ["vardoger","mcp"]`, expects host `$HOME` bind-mounted at `/host-home`). Tracked `server.yaml` and submission walkthrough live at `plugins/docker-mcp/`. Decision 2026-04-24: let Docker build and sign `mcp/vardoger` on Docker Hub (the default `task create --category productivity` path). Remaining work is gated on the 0.3.1 PyPI release so `source.commit` can be pinned to the release tag SHA before opening the PR against `docker/mcp-registry`; see [`MARKETPLACE_STATUS.md`](./MARKETPLACE_STATUS.md).
