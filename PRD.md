...
- [ ] Skill publishing to ClawHub for OpenClaw — **not started — owner action required** (confirmed 2026-04-22 that [clawhub.ai](https://clawhub.ai/search?q=vardoger) has no `vardoger` listing; publish requires `bun clawhub skill publish` with interactive GitHub OAuth sign-in on clawhub.ai, see [`MARKETPLACE_STATUS.md`](./MARKETPLACE_STATUS.md) for the exact command)
