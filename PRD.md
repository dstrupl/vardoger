...
- [ ] McpMux community registry submission ([`mcpmux/mcp-servers`](https://github.com/mcpmux/mcp-servers)) — **not started — prep complete** (draft `vardoger.json` at `plugins/mcpmux/vardoger.json`; submission is a signed-commit PR after `pnpm validate` and `pnpm check-conflicts`).
- [ ] Docker MCP Registry submission ([`docker/mcp-registry`](https://github.com/docker/mcp-registry)) — **not started — blocked on Dockerfile** (Docker's catalog requires a working image; no `Dockerfile` exists in the vardoger repo yet, so this is deferred pending a separate decision on whether to containerize).
