...
Last refreshed: **2026-04-22** (UTC). ClawHub flipped to **Live (self-served)** the same day as `vardoger-analyze@0.3.0`; Windsurf re-verified (still no public submission); three cross-vendor MCP registry rows added (`modelcontextprotocol/registry`, McpMux, Docker MCP Registry).
...
| **McpMux community registry** | PR to [`mcpmux/mcp-servers`](https://github.com/mcpmux/mcp-servers) | `plugins/mcpmux/` | Not started — prep complete | — | — | [McpMux](https://mcpmux.com) is a desktop MCP gateway that proxies tools into Cursor, Claude Desktop, VS Code, and Windsurf through one endpoint; one merged PR lands vardoger in every McpMux client. Draft `vardoger.json` at `plugins/mcpmux/vardoger.json` (schema version `2.1`, stdio transport invoking `vardoger mcp`, categories `developer-tools` / `productivity` / `ai-ml`, `platforms: ["all"]`). Submission flow per `plugins/mcpmux/README.md`: fork, copy the tracked file into `servers/io.github-dstrupl-vardoger.json` (upstream names every server file after its `id`), `pnpm validate servers/io.github-dstrupl-vardoger.json && pnpm check-conflicts`, open DCO-signed-commit PR `Add vardoger`. No PyPI re-release required — McpMux validates metadata only. Dry-run against the live upstream schema on 2026-04-22 confirmed `PASS` + no ID/alias conflicts across the current 106 servers. |
...
- **McpMux (`mcpmux/mcp-servers`)** — no PyPI work needed. Fork, copy
  `plugins/mcpmux/vardoger.json` to `servers/io.github-dstrupl-vardoger.json`
  (upstream names every server file after its `id`), run
  `pnpm install && pnpm validate servers/io.github-dstrupl-vardoger.json && pnpm check-conflicts`,
  then open a DCO-signed-commit PR titled `Add vardoger`. The tracked
  `$schema: "../schemas/server-definition.schema.json"` resolves as-is once
  the file is in `servers/`. Flip to **Submitted** on PR open and **Live** on
  merge.
