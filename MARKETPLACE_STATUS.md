...
| **OpenClaw ClawHub**        | `bun clawhub skill publish …` (requires interactive GitHub sign-in to [clawhub.ai](https://clawhub.ai)) | `plugins/openclaw/skills/analyze/` | Not started — owner action required | — | — | Verified 2026-04-22 via the read-only `clawhub` CLI: `bunx --bun clawhub inspect {vardoger, vardoger-analyze, vardoger-personalization}` all return `✖ Skill not found`, and `bunx --bun clawhub search {vardoger, dstrupl}` return zero results (sanity-checked against `inspect analyze`, which resolves to an unrelated existing skill). `SKILL.md` frontmatter already matches the ClawHub schema (`version`, `metadata.openclaw.requires.bins`, `homepage`). Publishing uses the `clawhub` CLI (run via `bun clawhub`) against the hosted clawhub.ai service with GitHub OAuth, so it needs a shell with browser access. From a fresh clone, run: `bun clawhub skill publish plugins/openclaw/skills/analyze/ --slug vardoger-analyze --name "vardoger — Analyze History" --version 0.3.0 --tags latest --changelog "Initial ClawHub release (vardoger 0.3.0)"`. Verification happens via the ClawHub dashboard after publish. |
