...
| **OpenClaw ClawHub**        | `bun clawhub publish …` (requires `clawhub login` — GitHub browser OAuth, or `--no-browser --token <API token>` generated at [clawhub.ai](https://clawhub.ai)) | `plugins/openclaw/skills/analyze/` | Not started — owner action required | — | — | Verified 2026-04-22 via the read-only `clawhub` CLI v0.9.0: `bunx --bun clawhub inspect {vardoger, vardoger-analyze, vardoger-personalization}` all return `✖ Skill not found`, and `bunx --bun clawhub search {vardoger, dstrupl}` return zero results (sanity-checked against `inspect analyze`, which resolves to an unrelated existing skill). `SKILL.md` frontmatter already matches the ClawHub schema (`version`, `metadata.openclaw.requires.bins`, `homepage`), and a dry publish from a non-authenticated shell confirms the folder and flags parse cleanly — the CLI fails only with `Error: Not logged in`. Publish command: `bun clawhub publish plugins/openclaw/skills/analyze/ --slug vardoger-analyze --name "vardoger — Analyze History" --version 0.3.0 --tags latest --changelog "Initial ClawHub release (vardoger 0.3.0)"`. Auth the CLI first with `bun clawhub login` (opens a browser) or `bun clawhub login --no-browser --token <TOKEN>` with an API token generated while signed in at clawhub.ai. Verification happens via the ClawHub dashboard after publish. |
